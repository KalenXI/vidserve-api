...
from .models import VideoModel, UpdateTaskModel

router = APIRouter()


@router.post("/", response_description="Add new video")
async def create_video(request: Request, task: VideoModel = Body(...)):
    task = jsonable_encoder(task)
    new_task = await request.app.mongodb["videos"].insert_one(task)
    created_task = await request.app.mongodb["videos"].find_one(
...
async def show_video(id: str, request: Request):
    if (task := await request.app.mongodb["videos"].find_one({"_id": id})) is not None:
        return task
...
async def update_video(id: str, request: Request, task: UpdateTaskModel = Body(...)):
    task = {k: v for k, v in task.dict().items() if v is not None}

    if len(task) >= 1:
        update_result = await request.app.mongodb["videos"].update_one(
            {"_id": id}, {"$set": task}
        )
